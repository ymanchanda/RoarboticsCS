...
public class ITDLiftSubsystem implements ISubsystem<ITDLiftStateMachine, ITDLiftStateMachine.State> {
    ...
    private static ITDLiftStateMachine itdLiftStateMachine;
    ...
    static {
        ...
        if(controllers.length == 2) {
            ...
            if(controllers[0].name().equals(ITDLiftStateMachine.State.EXTEND.getName())) {
    ...
    public ITDLiftSubsystem(RevMotor lift) {
        setLiftStateMachine(new ITDLiftStateMachine(this));
        setLift(lift);
        setLastError(0d);
        resetRunningSum();
    }
    ...
    @Override
    public ITDLiftStateMachine getStateMachine() {
        return itdLiftStateMachine;
    }

    @Override
    public ITDLiftStateMachine.State getState() {
    ...
    public void update(double dt) {
        ...
        double output;
        if(getLiftStateMachine().getState().equals(ITDLiftStateMachine.State.EXTEND)) { //changed from getstate to getdesiredstate
    ...
    public static ITDLiftStateMachine getLiftStateMachine() {
        return itdLiftStateMachine;
    }

    public static void setLiftStateMachine(ITDLiftStateMachine liftSM) {
        ITDLiftSubsystem.itdLiftStateMachine = liftSM;
    }
    ...
    public void setSetpoint(double setpoint) {
        ...
        if(setpoint != getSetpoint() && (getExtensionProfile() == null || getExtensionProfile().isDone())) {
            if(setpoint != 0d) {
                //Extending
                getLiftStateMachine().updateState(ITDLiftStateMachine.State.EXTEND);
                this.setpoint = setpoint;
            } else {
                //Retracting
                getLiftStateMachine().updateState(ITDLiftStateMachine.State.RETRACT);
                this.setpoint = setpoint;
    ...
    public static void setExtensionProfile(IMotionProfile extensionProfile) {
        ITDLiftSubsystem.extensionProfile = extensionProfile;
    }
    ...
    public static void setDesiredSetpoint(double desiredSetpoint) {
        ITDLiftSubsystem.desiredSetpoint = desiredSetpoint;
    }
}
